...
async def main():
    '''
    Run AMI136 IOC: load settings, create dispatcher, set name, start devices, do boilerplate
    '''
    ...
    p = ReadAMI136(device_name, settings['ami136'], records)
...
class ReadAMI136:
    '''Set up PVs for AMI 136 and connect to device
    '''
    ...
    def __init__(self, device_name, settings, records):
        ...
        self.thread = AMI136Thread(self)
        self.thread.daemon = True
        self.thread.start()


class AMI136Thread(Thread):
    ...
    def __init__(self, parent):
        ...
        if self.enable:  # if not enabled, don't connect
            self.t = AMI136(parent.settings['ip'], parent.settings['port'],
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                print(f"PV set failed: {e}, {self.values}")

    def reconnect(self):
        del self.t
        print("Connection failed. Attempting reconnect.")
        try:
            self.t = AMI136(self.settings['ip'], self.settings['port'],
